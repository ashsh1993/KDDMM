function [Rules, RuleSup, RuleConf, nTarget, nFeature] = findRules(transactions, minSup, minConf, nRules, sortFlag, labels, fname)
%
...
nTarget = zeros(nRules,1);
nFeature = zeros(nRules,1);
ct = 1;

% Find frequent item sets of size one (list of all items with minSup)
T = [];
idx = transactions(:,end)==1;
target_transactions = transactions(idx,:);
for i = 1:N-1
    S = sum(target_transactions(:,i))/size(target_transactions,1);
    if S >= minSup
        T = [T; i];
    end
end
...
for steps = 2:N
    ...
    Combinations = nchoosek(U',steps-1);
    Combinations = horzcat(Combinations, N*ones(size(Combinations,1),1));%Last column is the target columns
    TOld = T;
    T = [];
    ...
    for j = 1:size(Combinations,1)
        if ct > nRules
            ...
            if sum(ismember(nchoosek(Combinations(j,1:(end-1)),l_subset),TOld,'rows')) - steps+2>0
                % Calculate the support for the new itemset in the target
                % transactions
               
                S = mean((sum(target_transactions(:,Combinations(j,1:(end-1))),2)-steps+1)>=0);
                if S >= minSup
                    T = [T; Combinations(j,1:(end-1))];
                    
                    % Generate potential rules and check for minConf
                    for depth = 1:steps-1
                        R = nchoosek(Combinations(j,1:(end-1)),depth);%last column is target value
                        for r = 1:size(R,1)
                            if ct > nRules
                                break;
                            else
                                % Calculate the confidence of the rule
                                idx = sum(transactions(:,R(r,:)),2)==depth;
                                item_transactions = transactions(idx,:);
                                %Ctemp = S/mean((sum(transactions(:,R(r,:)),2)-depth)==0);
                                Ctemp = mean(item_transactions(:,end)==1);
                                if Ctemp > minConf
                                    ...
                                    RuleConf(ct) = Ctemp;
                                    RuleSup(ct) = S;
                                    nFeature(ct) = size(item_transactions,1);
                                    nTarget(ct) = size(target_transactions,1);
                                    ct = ct+1;
                                end
                            end
                        end
